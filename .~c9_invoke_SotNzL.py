...
@login_required
def sell():
    stockOpt = [(userCurrent.symbolName, userCurrent.noShares) for item in userCurrent.query.filter_by(user_id = current_user.id).order_by(userCurrent.symbolName).all()]
